...
const fetchPRDetails = async (prNumber) => {
    try {
        ...
        participants.delete('github-actions[bot]'); // Exclude 'github-actions[bot]'
        ...
        if (prData.merged) {
            const mergeResponse = await axios.get(`https://api.github.com/repos/ethereum/EIPs/pulls/${prNumber}/merge`, {
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                },
            });
            ...
            console.log('PR has not been merged yet.');
        }
    } catch (error) {
...
const prNumber = 7132; // Replace with the desired PR number
